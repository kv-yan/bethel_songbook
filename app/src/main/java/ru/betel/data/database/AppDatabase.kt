...
val MIGRATION_TEMPLATE_1_2 = object : Migration(1, 2) {
    override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL(
            "CREATE TABLE IF NOT EXISTS `template_temp` " + "(`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, " + "`create_date` TEXT NOT NULL, " + "`performer_name` TEXT NOT NULL, " + "`weekday` TEXT NOT NULL, " + "`isSingleMode` INTEGER NOT NULL, " + "`glorifyingSong` TEXT NOT NULL, " + "`worshipSong` TEXT NOT NULL, " + "`giftSong` TEXT NOT NULL, " + "`singleModeSongs` TEXT NOT NULL)"
        )

        database.execSQL(
            "INSERT INTO `template_temp` (" + "`id`, `create_date`, `performer_name`, `weekday`, `isSingleMode`, " + "`glorifyingSong`, `worshipSong`, `giftSong`, `singleModeSongs`) " + "SELECT `id`, `create_date`, `performer_name`, `weekday`, `isSingleMode`, " + "`glorifyingSong`, `worshipSong`, `giftSong`, `singleModeSongs` FROM `template`"
        )
